...
{
    public class PreValidateWorkOrderProduct : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.InputParameters.Contains("Target") && context.InputParameters["Target"] is Entity
                && context.PrimaryEntityName.ToLower() == "msdyn_workorderproduct" && (context.MessageName.ToUpper() == "CREATE"))
            {
                try
                {
                    ...
                    if (!entity.Contains("msdyn_workorderincident"))
                        throw new InvalidPluginExecutionException("Job Incident is required.");
                    ...
                    {  //Emergency Order and Approved
                        throw new Exception("Access Denied!!!.\nYou can't add Spare Part in Completed Job.");
                    }
                }
                catch (Exception ex)
                {
