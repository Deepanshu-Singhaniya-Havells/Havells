...
{
    public class PostCreatePurchaseReceiptLine : IPlugin
    {
        ...
        private void ProcessRequest(Entity entity, ITracingService _tracingService, IOrganizationService service)
        {
            try
            {
                Entity _poLineId = service.Retrieve("hil_inventorypurchaseorderreceiptline", entity.Id, new ColumnSet("hil_purchaseorderline"));

                string _query = $@"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false' aggregate='true'>
                    ...
                    <filter type='and'>
                    <condition attribute='hil_purchaseorderline operator='eq' value='{_poLineId.Id}' />
                    </filter>
                    </entity>
                    </fetch>";
