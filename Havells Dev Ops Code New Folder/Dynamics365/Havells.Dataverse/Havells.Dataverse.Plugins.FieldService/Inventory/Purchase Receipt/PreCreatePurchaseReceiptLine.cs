...
{
    public class PreCreatePurchaseReceiptLine : IPlugin
    {
        ...
        private void ProcessRequest(Entity entity, ITracingService _tracingService, IOrganizationService service)
        {
            try
            {
                Entity _headerId = service.Retrieve("hil_inventorypurchaseorderreceipt", entity.GetAttributeValue<EntityReference>("hil_receiptnumber").Id, new ColumnSet("hil_name"));
                ...
                <filter type='and'>
                    <condition attribute='hil_receiptnumber' operator='eq' value='{_headerId.Id}' />
                </filter>
                </entity>
                </fetch>";
