...
{
    public class PreValidatePurchaseOrder : IPlugin
    {
                ...
                && context.PrimaryEntityName.ToLower() == "hil_inventorypurchaseorder" && (context.MessageName.ToUpper() == "CREATE"))
            {
                try
                {
                    ...
                    if (!entity.Contains("hil_ordertype"))
                        throw new InvalidPluginExecutionException("Order Type is required.");

                    if (!entity.Contains("hil_brand"))
                        throw new InvalidPluginExecutionException("Brand is required.");

                    if (!entity.Contains("hil_franchise"))
                        throw new InvalidPluginExecutionException("Franchise is required.");
