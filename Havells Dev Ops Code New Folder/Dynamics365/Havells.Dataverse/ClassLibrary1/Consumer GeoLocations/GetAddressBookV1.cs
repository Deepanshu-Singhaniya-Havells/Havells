...
{
    public class GetAddressBookV1 : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            List<IoTAddressBookResultV1> lstAddressBookResult = new List<IoTAddressBookResultV1>();
            ...
            {
                lstAddressBookResult.Add(new IoTAddressBookResultV1
                {
                    StatusCode = 204,
            ...
            if (context.InputParameters.Contains("IsDefault"))
            {
            ...
            IoTAddressBookV1 address = new IoTAddressBookV1 { CustomerGuid = CustomerGuid, MobileNumber = MobileNumber, IsDefault = IsDefault, Pincode = Pincode };
            JsonResponse = JsonSerializer.Serialize(GetAddressBookV1Details(service, address));
            _tracingService.Trace(JsonResponse);
            context.OutputParameters["data"] = JsonResponse;
        }
        public List<IoTAddressBookResultV1> GetAddressBookV1Details(IOrganizationService service, IoTAddressBookV1 address)
        {
            ...
            try
            {
                if (service != null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(address.MobileNumber))
                    {
