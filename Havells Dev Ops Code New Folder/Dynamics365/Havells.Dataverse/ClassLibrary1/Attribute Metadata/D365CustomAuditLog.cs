...
{
    public class D365CustomAuditLog : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            try
            {
                if (!context.InputParameters.Contains("EntityName") ||
                    !context.InputParameters.Contains("RecId") ||
                    !context.InputParameters.Contains("FromDate") ||
                    !context.InputParameters.Contains("ToDate") ||
                    !context.InputParameters.Contains("FieldName"))
                {
                    context.OutputParameters["data"] = "Missing required input parameters.";
                    return;
                }
                var requestData = new D365AuditLog
                {
                    EntityName = context.InputParameters["EntityName"]?.ToString(),
                    RecId = context.InputParameters["RecId"]?.ToString(),
                    FromDate = context.InputParameters["FromDate"]?.ToString(),
                    ToDate = context.InputParameters["ToDate"]?.ToString(),
                    FieldName = context.InputParameters["FieldName"]?.ToString()
                };
                jsonResponse = JsonSerializer.Serialize(GetD365AuditLogData(requestData, service));
                context.OutputParameters["data"] = jsonResponse;
            }
            catch (Exception ex)
            {
                context.OutputParameters["data"] = ex.Message;
            }
        }
        public List<D365AuditLogResult> GetD365AuditLogData(D365AuditLog _requestData, IOrganizationService _service)
        {
            List<D365AuditLogResult> objD365AuditLog = new List<D365AuditLogResult>();
            try
            {

                D365AuditLogMigrationDaily(_requestData.EntityName, _requestData.RecId, _service);
        ...
        private DateTime ConvertStringToDate(string dateStr)
        {
            return new DateTime(
                ...
                int.Parse(dateStr.Substring(12, 2))
            );
        }
