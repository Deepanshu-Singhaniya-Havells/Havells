...
namespace ConsumerApp.BusinessLayer
{
    ...
    public class IoTAddressBook
    {
        ...
        public List<IoTAddressBookResult> GetIoTAddressBook(IoTAddressBook address)
        {
            ...
            try
            {
                ...
                if (service != null)
                {
                    ...
                    query.ColumnSet = new ColumnSet("hil_customer", "hil_addressid", "hil_street3", "hil_street2", "hil_street1", "hil_fulladdress", "hil_pincode", "hil_area", "hil_businessgeo", "hil_addresstype");
                    query.Criteria = new FilterExpression(LogicalOperator.And);
            ...
            try
            {
                ...
                if (service != null)
                {
                    ...
                    if (addressData.AddressTypeEnum == null)
                    {
                        ...
                        //if (entcoll.Entities.Count > 0) { addressType = 3; }
                        //else { addressType = 1; }
                        addressType = 3; //Other Address 
                    }
                    else if (addressData.AddressTypeEnum == "1") // Home
                    {
                        addressType = 1;
                    }
                    else if (addressData.AddressTypeEnum == "2") // Office
                    {
                        addressType = 2;
                    }
        ...
        public IoTAddressBookResult IoTUpdateAddress(IoTAddressBookResult addressData)
        {
            ...
            try
            {
                ...
                if (service != null)
                {
                    ...
                    {
                        entObj.hil_AddressType = new OptionSetValue(1); //Home
                    }
                    else if (addressData.AddressTypeEnum == "2")
                    {
                        entObj.hil_AddressType = new OptionSetValue(2); //Office
                    }
                    else {
                        entObj.hil_AddressType = new OptionSetValue(3); //Others
                    }
        ...
        public IoTAddressBookResult IoTDeleteAddress(IoTAddressBookResult addressData)
        {
            IoTAddressBookResult objAddress = null;
            QueryExpression query;
            EntityCollection entcoll;
            ...
            try
            {
                ...
                if (service != null)
                {
                    ...
                    if (entcoll.Entities.Count == 0)
                    {
                        objAddress = new IoTAddressBookResult { StatusCode = "204", StatusDescription = "Address does not exist in D365." };
                        return objAddress;
                    }
                    else {
                        if (entcoll.Entities[0].GetAttributeValue<OptionSetValue>("statecode").Value == 1) {
                            objAddress = new IoTAddressBookResult { StatusCode = "204", StatusDescription = "Address is already deleted." };
                            return objAddress;
                        }
                    }
                    ...
                    if (entcoll.Entities.Count > 0) {
                        objAddress = new IoTAddressBookResult { StatusCode = "204", StatusDescription = $"Address is in use against JobId:{entcoll.Entities[0].GetAttributeValue<string>("msdyn_name")}.Can't be deleted."};
                        return objAddress;
                    }
                    else {
                    ...
                    objAddress = new IoTAddressBookResult { StatusCode = "200", StatusDescription = "Success" };
                }
                else
                {
                    objAddress = new IoTAddressBookResult { StatusCode = "503", StatusDescription = "D365 Service Unavailable" };
