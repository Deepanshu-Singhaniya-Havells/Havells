...
namespace ConsumerApp.BusinessLayer
{
    ...
    public class Job
    {
        ...
        [DataMember]
        public string SourceOfJob { get; set; }

        public List<JobOutput> getJobs(Job job)
        {
            List<JobOutput> jobList = new List<JobOutput>();
            if (String.IsNullOrWhiteSpace(job.Job_ID) && String.IsNullOrWhiteSpace(job.MobileNumber))
            {
                return jobList;
            }
            if (SourceOfJob == "12")
            ...
            QueryExpression query = new QueryExpression()
            {
                ...
                ColumnSet = new ColumnSet("msdyn_customerasset", "msdyn_name", "msdyn_substatus", "hil_productcategory", "createdon", "hil_jobclosuredon", "hil_mobilenumber", "hil_fulladdress", "hil_customerref", "hil_owneraccount", "hil_callsubtype", "hil_customercomplaintdescription", "msdyn_customerasset", "hil_productcategory"),
                TopCount = 10
            };
