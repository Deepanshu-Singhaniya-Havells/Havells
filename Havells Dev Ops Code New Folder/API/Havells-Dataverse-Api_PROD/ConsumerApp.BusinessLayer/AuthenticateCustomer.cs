...
namespace ConsumerApp.BusinessLayer
{
    ...
    public class AuthenticateCustomer
    {
        ...
        public AuthenticateCustomer ValidateThisCustomer(AuthenticateCustomer bridge)
        {
            ...
            //        //Query.Criteria.AddCondition("hil_password", ConditionOperator.Equal, bridge.Pwd);
            ...
            //            //Query1.Criteria.AddCondition("hil_password", ConditionOperator.Equal, bridge.Pwd);
        ...
        public AuthenticateCustomer ValidateThisCustomerDE(AuthenticateCustomer bridge)
        {
            NewAPK = true;
            if (!NewAPK) throw new Exception();
            IOrganizationService service = ConnectToCRM.GetOrgService();//get org service obj for connection
            try
            {
                if (bridge.Method == "CustomerLogin")
                {
                    ...
                    Query.ColumnSet = new ColumnSet("emailaddress1", "hil_password", "firstname", "lastname", "mobilephone");
                    Query.Criteria = new FilterExpression(LogicalOperator.And);
                    Query.Criteria.AddCondition("emailaddress1", ConditionOperator.Equal, SecurityUtility.Decrypt(bridge.UName));
                    EntityCollection Found = service.RetrieveMultiple(Query);
                    if (Found.Entities.Count == 1)
                    {
                        Contact Cont = Found.Entities[0].ToEntity<Contact>();
                        if (Cont.hil_Password == SecurityUtility.Decrypt(bridge.Pwd))
                        {
                            bridge.ContGuid = SecurityUtility.Encrypt(Cont.ContactId.Value.ToString());
                            if (Cont.MobilePhone != null)
                                bridge.MobileNumber = SecurityUtility.Encrypt(Cont.MobilePhone);
                            bridge.IfValidated = true;
                            if (Cont.LastName != null)
                                bridge.LastName = SecurityUtility.Encrypt(Cont.LastName);
                            if (Cont.FirstName != null)
                                bridge.FirstName = SecurityUtility.Encrypt(Cont.FirstName);
                            bridge.PinCode = GetPermanentAddressPinCode(service, Cont.Id);
                            bridge.status = true;
                            bridge.ERROR_CODE = 0;
                        }
                    ...
                    else
                    {
                        ...
                        Query1.ColumnSet = new ColumnSet("emailaddress1", "hil_password", "firstname", "lastname", "mobilephone");
                        Query1.Criteria = new FilterExpression(LogicalOperator.And);
                        Query1.Criteria.AddCondition("mobilephone", ConditionOperator.Equal, SecurityUtility.Decrypt(bridge.UName));
                        EntityCollection Found1 = service.RetrieveMultiple(Query1);
                        if (Found1.Entities.Count == 1)
                        {
                            Contact Cont1 = Found1.Entities[0].ToEntity<Contact>();
                            if (Cont1.hil_Password == SecurityUtility.Decrypt(bridge.Pwd))
                            {
                                bridge.ContGuid = SecurityUtility.Encrypt(Cont1.Id.ToString());
                                if (Cont1.MobilePhone != null)
                                    bridge.MobileNumber = SecurityUtility.Encrypt(Cont1.MobilePhone);
                                bridge.IfValidated = true;
                                if (Cont1.LastName != null)
                                    bridge.LastName = SecurityUtility.Encrypt(Cont1.LastName);
                                if (Cont1.FirstName != null)
                                    bridge.FirstName = SecurityUtility.Encrypt(Cont1.FirstName);
                                bridge.PinCode = GetPermanentAddressPinCode(service, Cont1.Id);
                                bridge.status = true;
                                bridge.ERROR_CODE = 0;
                            }
        ...
        public string GetPermanentAddressPinCode(IOrganizationService service, Guid ContId)
        {
            ...
            if (Found.Entities.Count > 0)
            {
                ...
                Pin = SecurityUtility.Encrypt(iAddress.hil_PinCode.Name);
            }
            return Pin;
        }
    }
