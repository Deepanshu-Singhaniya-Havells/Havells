...
namespace ConsumerApp.BusinessLayer
{
    ...
    public class MRNEntry
    {
        ...
        public MRNHeader AddtoViewList(MRNEntry _productInfo)
        {
            ...
            try
            {
                ...
                if (service != null)
                {
                    ...
                    if (entCol.Entities.Count == 0)
                    {
                        SerialNumberValidationResult result = ValidateACInOutScan(_headerGuId, _productInfo, service);
                        if (result._returnFlag == false)
                        {
                            return new MRNHeader() { ResultStatus = result._returnFlag, ResultMessage = result._returnMsg };
                        }
        ...
        public MRNHeader SubmitViewList(MRNHeader _viewList)
        {
            ...
            try
            {
                ...
                if (service != null)
                {
                    ...
                    //    <entity name='hil_materialreturn'>
                    //    <attribute name='hil_istype' alias='rowcount' aggregate='count'/> 
                    //    <attribute name='hil_product' alias='modelcode' groupby='true' /> 
                    //    <attribute name='hil_istype' alias='istype' groupby='true' /> 
                    //    <filter type='and'>
                    //        <condition attribute='hil_dealerstockverificationheader' operator='eq' value='{_viewList.MRNGuId}' />
                    //        <filter type='or'>
                    //            <condition attribute='hil_istype' operator='eq' value='I' />
                    //            <condition attribute='hil_istype' operator='eq' value='O' />
                    //        </filter>
                    //    </filter>
                    //    </entity>
                    //    </fetch>";
                    ...
                    //    {
                    //        int _recordCnt = Convert.ToInt32(ent.GetAttributeValue<AliasedValue>("rowcount").Value);
                    //        if (_recordCnt % 2 != 0)
                    //        {
                    //            return _retObj = new MRNHeader() { ResultStatus = false, ResultMessage = (ent.GetAttributeValue<AliasedValue>("istype").Value.ToString() == "I" ? "Indoor" : "Outdoor") + " unit is missing for Model# " + ((EntityReference)ent.GetAttributeValue<AliasedValue>("modelcode").Value).Name+".", MRNGuId = _viewList.MRNGuId };
                    //        }
                    //    }
                    //}
                    ...
                    if (entcoll.Entities.Count > 0)
                    {
                        int _inDoorUnits = 0, _outDoorUnits = 0;
                        foreach (Entity ent in entcoll.Entities)
                        {
                            _fetchXML = $@"<fetch distinct='false' mapping='logical' aggregate='true'>
                                ...
                                <attribute name='hil_istype' alias='istype' groupby='true' /> 
                                <filter type='and'>
                                    <condition attribute='hil_product' operator='eq' value='{((EntityReference)ent.GetAttributeValue<AliasedValue>("modelcode").Value).Id}' />
                                    <condition attribute='statecode' operator='eq' value='0' />                            
                                    <filter type='or'>
                                        <condition attribute='hil_istype' operator='eq' value='I' />
                                        <condition attribute='hil_istype' operator='eq' value='O' />
                            ...
                            EntityCollection entcoll1 = service.RetrieveMultiple(new FetchExpression(_fetchXML));
                            if (entcoll1.Entities.Count < 2)
                            {
                                return _retObj = new MRNHeader() { ResultStatus = false, ResultMessage = "ERROR ! doesn't contains matching count of Indoor and Outdoor units in Model# " + ((EntityReference)ent.GetAttributeValue<AliasedValue>("modelcode").Value).Name };
                            }
                            else
                            {
                                _inDoorUnits = entcoll1.Entities[0].GetAttributeValue<AliasedValue>("istype").Value.ToString() == "I" ? Convert.ToInt32(entcoll1.Entities[0].GetAttributeValue<AliasedValue>("rowcount").Value.ToString()) : entcoll1.Entities[1].GetAttributeValue<AliasedValue>("istype").Value.ToString() == "I" ? Convert.ToInt32(entcoll1.Entities[1].GetAttributeValue<AliasedValue>("rowcount").Value.ToString()) : 0;
                                _outDoorUnits = entcoll1.Entities[0].GetAttributeValue<AliasedValue>("istype").Value.ToString() == "O" ? Convert.ToInt32(entcoll1.Entities[0].GetAttributeValue<AliasedValue>("rowcount").Value.ToString()) : entcoll1.Entities[1].GetAttributeValue<AliasedValue>("istype").Value.ToString() == "O" ? Convert.ToInt32(entcoll1.Entities[1].GetAttributeValue<AliasedValue>("rowcount").Value.ToString()) : 0;
                                if (_inDoorUnits != _outDoorUnits)
                                {
                                    return _retObj = new MRNHeader() { ResultStatus = false, ResultMessage = "ERROR ! doesn't contains matching count of Indoor and Outdoor units in Model# " + ((EntityReference)ent.GetAttributeValue<AliasedValue>("modelcode").Value).Name };
                                }
                            }
        ...
        public SerialNumberValidationResult ValidateACInOutScan(Guid _headerGuId, MRNEntry _productInfo, IOrganizationService service)
        {
            //SerialNumberValidationResult _retObj = null;
            try
            {
                QueryExpression qrExp;
                EntityCollection entCol;
                ...
                if (entCol.Entities.Count > 0)
                {
                    ...
                    if (IsType == "I" || IsType == "O")
                    {
                        ...
                        entCol = service.RetrieveMultiple(qrExp);
                        if (entCol.Entities.Count >= 2 && entCol.Entities.Count % 2 == 0)
                        {
                            return new SerialNumberValidationResult() { _returnFlag = true, _returnMsg = $"Success:  ProductCode-{_productInfo.ProductCode} IsType-{_productInfo.Is_Type}" };
                        }
                        else
                        {
                            if (IsType != _productInfo.Is_Type && (_productInfo.Is_Type == "I" || _productInfo.Is_Type == "O") && _productInfo.ProductCode == ModelCode)
                            {
                                return new SerialNumberValidationResult() { _returnFlag = true, _returnMsg = $"Success:  ProductCode-{_productInfo.ProductCode} IsType-{_productInfo.Is_Type}" };
                            }
                            else
                            {
                                return new SerialNumberValidationResult() { _returnFlag = false, _returnMsg = "Please Scan " + (IsType == "I" ? "Out door Unit " : "In Door Unit ") + " of Product " + ModelCode };
                            }
                        }
                    }
                    else {
            ...
            {
                return new SerialNumberValidationResult() { _returnFlag = false, _returnMsg = "ERROR! " + ex.Message };
            }
        }
    ...
    public class SerialNumberValidationResult {
        public bool _returnFlag { get; set; }
