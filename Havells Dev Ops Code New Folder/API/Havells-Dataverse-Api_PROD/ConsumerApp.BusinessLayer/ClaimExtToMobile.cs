...
namespace ConsumerApp.BusinessLayer
{
    ...
    public class ClaimExtToMobile
    {
        ...
        public WOSchemesResult PatchSchemeCode(ClaimExtToMobile _reqParam) {
            WOSchemesResult _retValue = new WOSchemesResult();
            try
            {
                #region Validate API IN Params
                if (_reqParam.WorkOrderId == Guid.Empty)
                {
                    _retValue = new WOSchemesResult() { ResultStatus = false, ResultMessage = "Work Order GuId is required." };
                    return _retValue;
                }
                if (_reqParam.SchemeCodeId == Guid.Empty)
                {
                    _retValue = new WOSchemesResult() { ResultStatus = false, ResultMessage = "Scheme Code GuId is required." };
                    return _retValue;
                }
                IOrganizationService service = null;
                service = ConnectToCRM.GetOrgService();
                if (service != null)
                {
                    Entity entUpdate = new Entity(msdyn_workorder.EntityLogicalName, _reqParam.WorkOrderId);
                    entUpdate["hil_schemecode"] = new EntityReference("hil_schemeincentive", _reqParam.SchemeCodeId);
                    service.Update(entUpdate);
                    _retValue = new WOSchemesResult() { ResultStatus = true, ResultMessage = "OK" };
                    return _retValue;
                }
                else {
                    _retValue = new WOSchemesResult() { ResultStatus = false, ResultMessage = "D365 Service Unavailable" };
                    return _retValue;
                }
                #endregion
            }
            ...
            {
                _retValue = new WOSchemesResult() { ResultStatus = false, ResultMessage = ex.Message };
            }
