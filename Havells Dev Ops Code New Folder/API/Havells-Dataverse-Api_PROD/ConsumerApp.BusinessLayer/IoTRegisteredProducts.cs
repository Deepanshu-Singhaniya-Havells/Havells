...
namespace ConsumerApp.BusinessLayer
{
    ...
    public class IoTRegisteredProducts
    {
        ...
        public IoTRegisteredProductsResult RegisterProductFromWhatsapp(IoTRegisteredProducts productData)
        {
            ...
            EntityReference erProductModel = new EntityReference("product", new Guid("4EE47D0A-9B5F-EA11-A811-000D3AF05D7B")); //OTHERS
            EntityReference erProductsubcategoryConfig = new EntityReference("hil_stagingdivisonmaterialgroupmapping", new Guid("FE765167-9B5F-EA11-A811-000D3AF05D7B")); //OTHERS
            try
            {
                IOrganizationService service = ConnectToCRM.GetOrgService();
                if (service != null)
                {
                    ...
                    if (productData.InvoiceAvailable)
                    {
                        ...
                        if (productData.SourceOfRegistration != 5) // Applicable for Other than Whatsapp Source
                        {
                    ...
                    if (IfExisting == false)
                    {
                        using (OrganizationServiceContext orgContext = new OrganizationServiceContext(service))
                        {
                            ...
                            entCustomerAsset["hil_source"] = new OptionSetValue(productData.SourceOfRegistration);
                            entCustomerAsset.hil_InvoiceAvailable = productData.InvoiceAvailable;
                            entCustomerAsset["hil_retailerpincode"] = "0";
                            entCustomerAsset["hil_purchasedfrom"] = "Registered From Whatsapp";

                            if (productData.InvoiceAvailable)
                            {
                                if (productData.InvoiceDate != null)
                                {
                                    //DateTime dtInvoice = DateTime.ParseExact(productData.InvoiceDate.ToString(), "yyyy-MM-ddTHH:mm:ss", CultureInfo.InvariantCulture);
        ...
        public IoTRegisteredProductsResult IoTRegisterProduct(IoTRegisteredProducts productData)
        {
            ...
            try
            {
                ...
                if (service != null)
                {
                    ...
                    {
                        //EntityReference erProductModel = new EntityReference("product", new Guid("4EE47D0A-9B5F-EA11-A811-000D3AF05D7B")); //OTHERS
                        //objRegisteredProductResult = new IoTRegisteredProductsResult { StatusCode = "204", StatusDescription = "Model Number is required." };
                        //return objRegisteredProductResult;
                        productData.ProductCode = "OTHER MODEL";
                    }
                    if (productData.InvoiceAvailable)
                    {
                        if (productData.InvoiceNumber == null || productData.InvoiceNumber.Trim().Length == 0)
