        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            try
            {
                  ...
                  && context.PrimaryEntityName.ToLower() == "incident" && context.Depth == 1)
                {
                    ...
                    //tracingService.Trace("fieldName1232 : " + fieldName);
                    if (fieldName != string.Empty)
                    {
                        //tracingService.Trace("caseNumber : " + caseNumber);
                        //tracingService.Trace("12 fieldName : " + fieldName);
                        entity[fieldName] = caseNumber;

                    }
                    //throw new InvalidPluginExecutionException("caseNumber : " + caseNumber);
                    ...
                    if (entity.Contains("customerid"))
                    {
                        if (entity.GetAttributeValue<EntityReference>("customerid").LogicalName == "account")
                        {
                            ...
                            //if (account.Contains("telephone1"))
                            //    entity["hil_alternatemobileno"] = account["telephone1"];
                        }
                    }
                    else
                    {
