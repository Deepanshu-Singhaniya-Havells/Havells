...
namespace HavellsNewPlugin.TenderModule.MailtoTeams
{
    ...
    public class SendEmailtoteamsonOA : CodeActivity
    {
        ...
        protected override void Execute(CodeActivityContext executionContext)
        {
            try
            {
                ...
                sendEmailonEventBasis(service, regardingRef, to, cc, mailBody, mailsubject, fromRef, tracingService);
        ...
        public static void sendEmailonEventBasis(IOrganizationService service, EntityReference regardingRef, string to, string cc,
            string mailBody, string mailsubject, EntityReference fromRef, ITracingService tracingService)
        {
            ...
            if (_regardingEntity.Contains("hil_orderchecklistid"))
            {
                ...
                tracingService.Trace("ocl is " + ocl.Name);
            }
            ...
            foreach (string totype in toTeam)
            {
                if (totype.Contains("t.") || totype.Contains("T."))
                {
                    ...
                    toTeamMembers = retriveTeamMembers(service, teamName, materialGroup, department, plant, toTeamMembers, tracingService);
                }
            ...
            foreach (string totype in ccTeam)
            {
                if (totype.Contains("t.") || totype.Contains("T."))
                {
                    string teamName = (totype.Replace("t.", "")).Replace("T.", "");
                    tracingService.Trace("team name" + teamName);
                    ccTeamMembers = retriveTeamMembers(service, teamName, materialGroup, department, plant, ccTeamMembers, tracingService);
                }
                else if (totype.Contains("p.") || totype.Contains("P."))
                {
                    string position = (totype.Replace("p.", "")).Replace("P.", "");
        ...
        static public EntityCollection retriveTeamMembers(IOrganizationService service, string _teamName,
            EntityReferenceCollection _materialGroup, EntityReference _department,
              EntityReference _plant, EntityCollection extTeamMembers, ITracingService tracingService)
        {
            try
            {
