...
{
    public class PaymentReceiptTreasury : IPlugin
    {
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            try
            {
                if (context.InputParameters.Contains("PaymentId") && context.InputParameters["PaymentId"] is string && context.Depth == 1)
                {
                    ...
                    #region update paymentterm
                    foreach (string guid in ArrPymentId)
                    {
                        Entity tenderpaymentdetail = new Entity("hil_tenderpaymentdetail");
                        tenderpaymentdetail.Id = new Guid(guid);
                        tenderpaymentdetail.Attributes["hil_status"] = new OptionSetValue(2);
                        service.Update(tenderpaymentdetail);
                    }
                    #endregion update paymentterm
                    ...
                    if (payCol.Entities.Count > 0)
                    {
                        ...
                        entCC["partyid"] = toemailer.GetAttributeValue<EntityReference>("hil_cmt");
                        entCCList.Entities.Add(entCC);
                        tracingService.Trace("6");
                        string subject = @"Confirmation of Payment Receipt";
                        EntityReference oaheaderregarding = payCol.Entities[0].GetAttributeValue<EntityReference>("hil_oaheader");
                        mailCommonConfig.sendEmal(owner, entCCList, oaheaderregarding, mailBody, subject, service);
                    }
                }
        ...
        public string mailBodtText(EntityCollection entcoll)
        {
            ...
            sbtopBody.Append("<Div style='width:700pt;margin-left:-.15pt;font-weight:bold'>Dear Sales team,</Div>");
            sbtable.Append("<Div><p>We hereby confirm the recipt of below Payment for material supplied against OA# " + entcoll.Entities[0].GetAttributeValue<EntityReference>("hil_oaheader").Name + ".  Check your payment Details </p></Div>");
            sbtable.Append("<Div>");
