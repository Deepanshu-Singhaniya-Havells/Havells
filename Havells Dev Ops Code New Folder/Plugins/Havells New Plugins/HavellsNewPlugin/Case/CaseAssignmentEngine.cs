    ...
    class CaseAssignmentEngine
    {
        public string AssignCase(IOrganizationService service, Entity _caseEntity)
        {
            ...
            //{
                ...
                EntityReference entBranch = entity.Contains("hil_branch") ? entity.GetAttributeValue<EntityReference>("hil_branch") : null;
                ...
                else
                {
                    ...
                    EntityReference entCaseProduct = entity.Contains("productid") ? entity.GetAttributeValue<EntityReference>("productid") : null;
                    ...
                    _fetchXML = $@"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
                        ...
                        <filter type='and'>
                        <condition attribute='hil_department' operator='eq' value='{entDepartment.Id}' />";
                        if (entDepartment.Id == CaseConstaints._serviceDepartment)
                            _fetchXML = _fetchXML + $@"<condition attribute='hil_branch' operator='eq' value='{entBranch.Id}' />";
                        _fetchXML = _fetchXML + $@"<condition attribute='statecode' operator='eq' value='0' />
                    ...
                    if (_entColl.Entities.Count > 0)
                    {
                        ...
                        caseEntity["hil_assignmentlevel"] = new OptionSetValue(1); // Level 1
                        caseEntity["hil_pendinglevel"] = new OptionSetValue(2); // Pending at Level 1 no first response
                        _returnMsg = "Case has been assigned to concern person.";
                    }
                    else
                    {
                        caseEntity["hil_pendinglevel"] = new OptionSetValue(1); // Pending for assignment 
                        _returnMsg = "Case Assignment Matrix doesn't exist in System.";
                    }
        ...
        public void CreateGrievanceHandlingActivity(IOrganizationService _service, Entity _caseEntity, string _caseNumber, string _subject, Entity _caseAssignmentMatrixLine, DateTime _caseResolveBy, EntityReference _assignee,int _activityType)
        {
            try
            {
                Entity _entCase = _service.Retrieve(_caseEntity.LogicalName, _caseEntity.Id, new ColumnSet("title", "ticketnumber"));

                Entity _grievanceActivity = new Entity("hil_grievancehandlingactivity");
                _grievanceActivity["subject"] = "Case ID " + _entCase.GetAttributeValue<string>("ticketnumber") + " regarding concern \'" + _entCase.GetAttributeValue<string>("title") + "\' has been assigned to you. Please resolve by " + _caseResolveBy.ToString("dd-MM-yyyy HH:mm:ss");
        ...
        private void AssignCase(IOrganizationService _service, Entity _entity, EntityReference _assignee, EntityReference _assignmentMatrix, DateTime _firstResponseBy, DateTime _caseResolveBy)
        {
            ...
            //{
                ...
                TimeSpan _diffMin = _caseResolveBy - DateTime.Now.AddMinutes(330);

                if (_diffMin.TotalMinutes <= 60) {
                    _entCase["hil_resolvebyduration"] = Convert.ToInt32(_diffMin.TotalMinutes) + " Minutes";
                }
                else if (_diffMin.TotalMinutes < 1440)
                {
                    ...
                    {
                        _entCase["hil_resolvebyduration"] = Convert.ToInt32(_diffMin.TotalMinutes) / 60 + 1 + " hrs";
                    }
                    else
                    _entCase["hil_resolvebyduration"] = Math.Round(_hrs,0) + " hrs";
                }
                else
                {
                    int _hr = Convert.ToInt32(_diffMin.TotalMinutes) / 1440;
                    _entCase["hil_resolvebyduration"] = _hr + " days";
                }
