            ...
            try
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (!string.IsNullOrEmpty(p1.participantAddress))
                        {
                            p1.participantAddress = p1.participantAddress.Substring(p1.participantAddress.Length - 10);
                        ...
                        if (!string.IsNullOrEmpty(p2.participantAddress))
                        {
                            p2.participantAddress = p2.participantAddress.Substring(p2.participantAddress.Length - 10);

                            query = new QueryExpression("contact");
                            query.ColumnSet = new ColumnSet("mobilephone");
                            query.Criteria.AddCondition("mobilephone", ConditionOperator.Equal, p2.participantAddress);
                            usersCollection = service.RetrieveMultiple(query);
                            if (usersCollection.Entities.Count > 0)
                            {
