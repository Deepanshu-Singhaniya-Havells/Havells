...
{
    public class PostUpdate_ResyncSecurityRole : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            try
            {
                    ...
                    && context.PrimaryEntityName.ToLower() == "systemuser" && context.MessageName.ToUpper() == "UPDATE")
                {
                    ...
                    if (entColl.Entities.Count > 0)
                    {
                        ...
                        if (entColl.Entities[0].Attributes.Contains("hil_resyncsecurityroles"))
                        {
                            reSyncSR = entColl.Entities[0].GetAttributeValue<bool>("hil_resyncsecurityroles");
                        }
            ...
            {
                throw new InvalidPluginExecutionException("Havells_Plugin.SystemUsers.PostUpdate_ResyncSecurityRole " + e.Message);
            }
            catch (Exception e)
            {
                throw new InvalidPluginExecutionException("Havells_Plugin.SystemUsers.PostUpdate_ResyncSecurityRole " + e.Message);
