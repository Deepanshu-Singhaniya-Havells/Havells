...
namespace Havells_Plugin.WorkOrderIncident
{
    public class PostUpdate : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            {
                throw new InvalidPluginExecutionException("Havells_Plugin.WorkOrderIncident.PostUpdate" + ex.Message);
            }
        }
                ...
                if (iHeaderId != Guid.Empty)
                {
                    ...
                    if (ipoLine != Guid.Empty && Brand == 1)
                    {
                        if (Servicable == 2)//IN WARRANTY NON-SERVICABLE //Straight Approve, No Approval required
                        {
                            ...
                            service.Update(Header);
                            service.Update(Line);
                            Havells_Plugin.JobStatuses.StatusTransition.setWoSubStatusRecord("Work Initiated", Wo.msdyn_WorkOrder.Id, service);
                        }
