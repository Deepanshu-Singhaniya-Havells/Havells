...
namespace Havells_Plugin.WOProduct
{
    public class PostUpdate_JobProduct_ReplacedPart_Warranty : IPlugin
    {
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            try
            {
                    ...
                    && context.PrimaryEntityName.ToLower() == msdyn_workorderproduct.EntityLogicalName.ToLower() && context.MessageName.ToUpper() == "UPDATE" && context.Depth == 1)
                {
                    ...
                    if (ent.Contains("hil_replacedpart") && ent.Attributes.Contains("hil_replacedpart"))
                    {
                        ...
                        if (ent.Contains("hil_markused") && ent.Attributes.Contains("hil_markused"))
                        {
                            if (ent.GetAttributeValue<bool>("hil_markused")) {
                                Entity entProd = service.Retrieve(Product.EntityLogicalName, ent.GetAttributeValue<EntityReference>("hil_replacedpart").Id, new ColumnSet("hil_hierarchylevel"));
                                Entity entJob = service.Retrieve(msdyn_workorder.EntityLogicalName, ent.GetAttributeValue<EntityReference>("msdyn_workorder").Id, new ColumnSet("hil_callsubtype"));
                                if (entProd != null && entJob != null)
                                {
                                    int sparePartHierarchyLevel = entProd.GetAttributeValue<OptionSetValue>("hil_hierarchylevel").Value;
                                    Guid callSubTypeGuid = entJob.GetAttributeValue<EntityReference>("hil_callsubtype").Id;
                                    if (callSubTypeGuid.ToString() == "55A71A52-3C0B-E911-A94E-000D3AF06CD4" && sparePartHierarchyLevel != 910590001)
                                    {
                                        throw new InvalidPluginExecutionException(" ***You are not authorized to use Spare Part with this Call Sub Type, please cordinate with Head Office.*** ");
                                    }
                                    else if (callSubTypeGuid.ToString() != "55A71A52-3C0B-E911-A94E-000D3AF06CD4" && sparePartHierarchyLevel == 910590001)
                                    {
                                        throw new InvalidPluginExecutionException(" ***You are not authorized to use AMC with this Call Sub Type, please cordinate with Head Office.*** ");
                                    }
                                }
                            }
                        }
            ...
            {
                throw new InvalidPluginExecutionException("Havells_Plugin.WorkOrderProduct.ReplacedPart_Warranty" + ex.Message);
            }
            #endregion
        }
        #endregion
