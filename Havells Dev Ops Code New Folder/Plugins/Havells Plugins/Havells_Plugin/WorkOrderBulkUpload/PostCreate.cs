...
namespace Havells_Plugin.WorkOrderBulkUpload
{
    public class PostCreate : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.InputParameters.Contains("Target")
                ...
                && context.PrimaryEntityName.ToLower() == "hil_bulkjobsuploader"
                && context.MessageName.ToUpper() == "CREATE" && context.Depth == 1)
            {
                ...
                Entity _updateStatus = new Entity(entity1.LogicalName, entity1.Id);
                ...
                try
                {
                    ...
                    if (inputsValidated)
                    {
                        entTemp = ExecuteScalar(service, "hil_consumercategory", "hil_name", "End User", new string[] { "hil_consumercategoryid" }, ref _updateStatus);
                        if (entTemp != null)
                        ...
                        else
                        {
                            ...
                            {
                                _updateStatus["hil_description"] = "Error While Creating Customer!!! " + ex.Message;
                                _updateStatus["hil_jobstatus"] = false;
                            }
                        }
                        if (!_duplicateWO)
                        {
                                ...
                                if (erAddress == null)
                                {
                                    ...
                                    if (entPincode != null)
                                    {
                                        ...
                                        if (entCol.Entities.Count >= 1)
                                        {
                                            ...
                                            {
                                                _updateStatus["hil_description"] = "Error While Creating Consumer's Address." + ex.Message;
                                                _updateStatus["hil_jobstatus"] = false;
                                            }
                                        }
                                        else
                                        {
                                            _updateStatus["hil_description"] = "No Business Geo Mapping found for PIN Code.";
                                            _updateStatus["hil_jobstatus"] = false;
                                        }
                                    }
                                    else
                                    {
                                        _updateStatus["hil_description"] = "PIN Code does not exist.";
                                        _updateStatus["hil_jobstatus"] = false;
                                    }
                                }
                            }
                            ...
                            if (entConsumer != null && erAddress != null)
                            {
                                ...
                                if (entTemp != null)
                                {
                                    ...
                                    //bool checkCallSubType = GetCallSubTypeByNOC(service, entTemp.Id, erNatureOfComplaint.Id);
                                    //if (entTemp != null && checkCallSubType)
                                    //{
                                    if (entTemp != null)
                                    {
                                        erCallSubType = entTemp.ToEntityReference();
                                    }
                                    //if (erConsumerCategory != null && erConsumerType != null)
                                    //{
                                    ...
                                    _updateStatus["hil_jobid"] = new EntityReference("msdyn_workorder", jobId);
                                    _updateStatus["hil_jobstatus"] = true;
                                    _updateStatus["hil_description"] = "Job created successfully.";
                                }
                                else
                                {
                                    _updateStatus["hil_description"] = "Product Sub Category Configuration (MG4) is missing.";
                                    _updateStatus["hil_jobstatus"] = false;
                                }
                            }
                            else
                            {
                                if (entConsumer == null)
                                {
                                    _updateStatus["hil_description"] = "Customer does not exist.";
                                    _updateStatus["hil_jobstatus"] = false;
                                }
                                else if (erAddress == null)
                                {
                                    _updateStatus["hil_description"] = addressRemarks + " Customer Address does not exist.";
                                    _updateStatus["hil_jobstatus"] = false;
                                }
                            }
                        }
                        ...
                        {
                            _updateStatus["hil_description"] = "Duplicate Job. Old Job# " + _duplicateWOIds;
                            _updateStatus["hil_jobstatus"] = false;
                        }
                    }
                    else
                    {
                        _updateStatus["hil_description"] = strInputsValidationsummary;
                        _updateStatus["hil_jobstatus"] = false;
                    }
                    #endregion
                ...
                {
                    _updateStatus["hil_description"] = "ERROR!! " + ex.Message;
                    _updateStatus["hil_jobstatus"] = false;
                    //throw new InvalidPluginExecutionException("  ***Havells_Plugin.WorkOrderBulkUpload.PostCreate.Execute***  " + ex.Message);
                }
                finally{ service.Update(_updateStatus);}
            }
        }
        public EntityReference GetCustomerAddress(IOrganizationService service, Guid customerGuid, Guid _pincode)
        {
            ...
            {
                throw new InvalidPluginExecutionException("  ***Havells_Plugin.WorkOrderBulkUpload.PostCreate.GetCustomerAddress***  " + ex.Message);
            }
            return retValue;
        }
        public EntityReference GetNatureOfComplaint(IOrganizationService service, string noc, Guid relatedProduct)
        {
            ...
            {
                throw new InvalidPluginExecutionException("  ***Havells_Plugin.WorkOrderBulkUpload.PostCreate.GetNatureOfComplaint***  " + ex.Message);
            }
            return retValue;
        }
        public Entity ExecuteScalar(IOrganizationService service, string entityName, string primaryField, string primaryFieldValue, string[] columns, ref Entity entity)
        {
        ...
        public bool GetCallSubTypeByNOC(IOrganizationService service, Guid callSubTypeGuid, Guid nocGuid)
        {
            ...
            {
                throw new InvalidPluginExecutionException("  ***Havells_Plugin.WorkOrderBulkUpload.PostCreate.GetCallSubTypeByNOC***  " + ex.Message);
            }
            return retValue;
        }
    }
