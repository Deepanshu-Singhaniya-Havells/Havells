...
namespace Havells_Plugin.WorkOrderBulkUpload
{
    public class PreCreate : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            try
            {
                    ...
                    && context.MessageName.ToUpper() == "CREATE")
                {
                    ...
                    if (entTemp != null) { erPinCode = entTemp.ToEntityReference(); } else { strInputsValidationsummary += "\n PIN Code does not exist."; inputsValidated = false; }

                    entTemp = ExecuteScalar(service, "hil_area", "hil_name", hil_area, new string[] { "hil_areaid" }, ref entity);
                    if (entTemp != null) { erArea = entTemp.ToEntityReference(); }
                    ...
                    if (entTemp != null) { erProductSubCategoryStagging = entTemp.ToEntityReference(); } else { strInputsValidationsummary += "\n Product Sub Category Mapping does not exist."; inputsValidated = false; }
                    ...
                    if (inputsValidated)
                    {
                        ...
                        erAddress = GetCustomerAddress(service, _customerGuid);
                        if (erAddress == null) {
                        ...
                        if (entTemp != null) { erArea = entTemp.ToEntityReference(); }
                        entTemp = ExecuteScalar(service, "hil_pincode", "hil_name", hil_pincode, new string[] { "hil_pincodeid" }, ref entity);
                        if (entTemp != null) { erPinCode = entTemp.ToEntityReference(); } else { addressRemarks = "PIN Code not exist."; }
                        ...
                        if (entCol.Entities.Count == 1)
                        {
                            entConsumer = entCol.Entities[0];
                            erAddress = GetCustomerAddress(service, entCol.Entities[0].Id);
                        }
                        else
                        {
                            string[] fullName = hil_customerfirstname.Split(' ');
                        ...
                        if (erAddress == null && entConsumer != null)
                        {
                            entTemp = ExecuteScalar(service, "hil_area", "hil_name", hil_area, new string[] { "hil_areaid" }, ref entity);
                            if (entTemp != null) { erArea = entTemp.ToEntityReference(); } else { addressRemarks = "Area does not exist in Area Master."; }
                            entTemp = ExecuteScalar(service, "hil_pincode", "hil_name", hil_pincode, new string[] { "hil_pincodeid" }, ref entity);
                            ...
                            if (entTemp != null)
                            {
                                ...
                                if (erNatureOfComplaint != null)
                                {
                                    ...
                                    if (entTemp != null && checkCallSubType)
                                    {
                                        ...
                                        {
                                            entity["hil_description"] = "Consumer Type/Category does not exist.";
                                            entity["hil_jobstatus"] = false;
                                        }
                                    }
                                    else
                                    {
                                        if (!checkCallSubType)
                                        {
                                            entity["hil_description"] = "Call Sub Type does not match with Nature of Complaint.";
                                            entity["hil_jobstatus"] = false;
                                        }
                                        else
                                        {
                                            entity["hil_description"] = "Call Sub Type does not exist.";
                                            entity["hil_jobstatus"] = false;
                                        }
                                    }
                                }
                                ...
                                {
                                    entity["hil_description"] = "Nature of Complaint not defined against Prod Sub Category.";
                                    entity["hil_jobstatus"] = false;
                                }
                            }
                            else
                            {
                                entity["hil_description"] = "Error While Fetching Product Sub Category Details.";
                                entity["hil_jobstatus"] = false;
                            }
                        }
                        else
            ...
            {
                throw new Exception("  ***Havells_Plugin.WorkOrderBulkUpload.PostCreate.Execute***  " + ex.Message);
            }
        }
        public EntityReference GetCustomerAddress(IOrganizationService service, Guid customerGuid)
        {
            ...
            {
                throw new Exception("  ***Havells_Plugin.WorkOrderBulkUpload.PreCreate.GetCustomerAddress***  " + ex.Message);
            }
            return retValue;
        }
        public EntityReference GetNatureOfComplaint(IOrganizationService service, string noc, Guid relatedProduct)
        {
            ...
            {
                throw new Exception("  ***Havells_Plugin.WorkOrderBulkUpload.PreCreate.GetNatureOfComplaint***  " + ex.Message);
            }
            return retValue;
        }
        ...
        public bool GetCallSubTypeByNOC(IOrganizationService service, Guid callSubTypeGuid, Guid nocGuid)
        {
            ...
            {
                throw new Exception("  ***Havells_Plugin.WorkOrderBulkUpload.PreCreate.GetCallSubTypeByNOC***  " + ex.Message);
            }
            return retValue;
        }
    }
