...
namespace JobProductInventoryPlugin
{
    public class Class1 : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
                    ...
                    && context.PrimaryEntityName.ToLower() == msdyn_workorderproduct.EntityLogicalName.ToLower() && context.MessageName.ToUpper() == "UPDATE" && context.Depth == 1)
            {
                ...
                if (postImageWOProduct.hil_Quantity != null && postImageWOProduct.hil_Quantity.Value > 0 && postImageWOProduct.hil_replacedpart != null)
                {
                    ...
                    int iQuantityRequired = Convert.ToInt32(postImageWOProduct.hil_Quantity.Value);
                    tracingService.Trace("Line 58 ");
        ...
        private Guid CreateInventoryJournal(Guid fsPart, Guid fsOwnerAccount, Guid fsOwner, Guid fsWOProductId, Guid fsGRNLineId, Guid fsInvReqId, Guid fsProductRequest, Guid fsReturnLineId, IOrganizationService service, ITracingService tracing, Int32 iAvailableQtyChange = 0, Int32 iDefectiveQtyChange = 0)
        {
            try
            {
        ...
        public static AccountOwnerCombo GetOwnerAccount(IOrganizationService service, msdyn_workorderproduct enWOProduct)
        {
            ...
            using (OrganizationServiceContext orgContext1 = new OrganizationServiceContext(service))
            {
                var obj = (from _woProduct in orgContext1.CreateQuery<msdyn_workorderproduct>()
                           join _WorkOrder in orgContext1.CreateQuery<msdyn_workorder>() on _woProduct.msdyn_WorkOrder.Id equals _WorkOrder.Id
                           // join _Account in orgContext.CreateQuery<Account>() on _WorkOrder.msdyn_ServiceAccount.Id equals _Account.Id
                           join _Account in orgContext1.CreateQuery<Account>() on _WorkOrder.hil_OwnerAccount.Id equals _Account.Id
                           join _Owner in orgContext1.CreateQuery<SystemUser>() on _Account.OwnerId.Id equals _Owner.Id
