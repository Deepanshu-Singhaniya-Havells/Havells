...
namespace InventoryWebJobs
{
    class SAPAPI
    {
        ...
        {
            Entity inventoryRMA = service.Retrieve("hil_inventoryrma", new Guid("50a3515c-eb4f-ef11-acce-7c1e520efe1f"), new ColumnSet("hil_name", "hil_franchise", "hil_productdivision", "hil_productdivision", "hil_returntype", "createdon"));
            ...
            var apiRequest = new HttpRequestMessage(HttpMethod.Post, "https://middlewareqa.havells.com:50001/RESTAdapter/d365/franchisee_return/insp_creation");
            apiRequest.Headers.Add("Authorization", "Basic RDM2NV9IQVZFTExTOlFBRDM2NUAxMjM0");
            ...
            if (Found.Entities.Count > 0)
            {
                ...
                foreach (Entity purchaseOrderP in Found.Entities)
                {
                    try
                    {
                        ...
                        //->// Remove the line 
                        Entity purchaseOrder = service.Retrieve("hil_inventorypurchaseorder", new Guid("8b0b6b46-ec4f-ef11-acce-7c1e520efe1f"), new ColumnSet(true));
                        // if(true)
        ...
        private Guid GetPurchaseOrderReceiptLine(Guid purchaseOrderReceipt, EntityReference product, EntityReference purchaseOrder, EntityReference freshWarehouse)
        {
        ...
        internal void GetSapInvoicetoSync()
        {
            ...
            if (resp.LT_INVOICE.Count > 0)
            {
                foreach (LTINVOICE lTINVOICE in resp.LT_INVOICE)
                {
                    try
                    {
                        if (lTINVOICE != null && lTINVOICE.AUBEL == "0112969534")
                        {
                            ...
                            if (product != null && franchise != null)
                            {
                                ...
                                if (purchaseOrderCollection.Entities.Count > 0 && wareHouseCollection.Entities.Count > 0)
                                {
                                    ...
                                    Guid purchaseOrderReceiptLine = GetPurchaseOrderReceiptLine(purchaseOrderReceipt, product, purchaseOrderCollection.Entities[0].ToEntityReference(), wareHouseCollection.Entities[0].ToEntityReference());
    ...
    internal class PostCreateUpdateJob
    {
        ...
        private Guid workDone = new Guid("2927fa6c-fa0f-e911-a94e-000d3af060a1"); // workDoneSubStauts
        ...
        internal void ToCall()
        {
            ...
            data.WorkOrder = service.Retrieve("msdyn_workorder", new Guid("f2aa01fd-564e-ef11-accd-6045bd71e09a"), new ColumnSet("hil_flagpo", "msdyn_substatus", "hil_owneraccount", "hil_productcategory", "hil_salesoffice", "hil_brand"));
    ...
    internal class TestingInventory
    {
        ...
        internal async Task ToCall()
        {
            ...
            SAPAPI sapAPIObj = new SAPAPI(service);
            sapAPIObj.SyncPoToSAP();

            sapAPIObj.GetSapInvoicetoSync();
            await sapAPIObj.PostRMA();
