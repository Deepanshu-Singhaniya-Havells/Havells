...
namespace SOPaymentReceipt
{
    public class ClsSalesOrder
    {
        ...
        public void RePushSalesOrderToSAP(Guid SalesOrderId)
        {
                ...
                string fetchXML = $@"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
                      <entity name='salesorder'>
                        ...
                        <filter type='and'>
                            ...
                            <condition attribute='hil_issynctosap' operator='eq' value='1' />
                            ...
                            foreach (Entity orderline in entOrderline.Entities)
                            {
                                if (orderline.Contains("productid"))
                                {
                                    ...
                                    if (Orddrtype != "A La Carte Sale")
                                    {
                                        ...
                                        {
                                            Entity entOrderUpdate = new Entity(ent.LogicalName, ent.Id);
                                            entOrderUpdate["hil_sapsyncmessage"] = $"Customer Asset not found on order.";
                                            entOrderUpdate["hil_issynctosap"] = false;
                                            _service.Update(entOrderUpdate);
                                            continue;
                                        }
                                        ...
                                        WarrantyDate warrantyDate = GetWarrantyStartAndEndDate(CustomerAsset.Id, ConsumerRef.Id, ent.Id);
                                        string AMCStartDate = warrantyDate.StartDate;
                                        string AMCEndDate = warrantyDate.EndDate;
                                        if (warrantyDate.StartDate == null || warrantyDate.EndDate == null)
                                        {
                                            Entity entOrderUpdate = new Entity(ent.LogicalName, ent.Id);
                                            entOrderUpdate["hil_sapsyncmessage"] = $"Unit Warranty line not found.";
                                            entOrderUpdate["hil_issynctosap"] = false;
                                            _service.Update(entOrderUpdate);
                                            Console.WriteLine($"Unit Warranty line not found for Asset Id: {CustomerAsset.Id}.");
                                            continue;
                                        }
                                        ...
                                        //{
                                        //    Entity entOrderUpdate = new Entity(ent.LogicalName, ent.Id);
                                        //    entOrderUpdate["hil_sapsyncmessage"] = $"Warranty Template not found for AMC Plan: {entRefProduct.Name}";
                                        //    entOrderUpdate["hil_issynctosap"] = false;
                                        //    _service.Update(entOrderUpdate);
                                        ...
                                        {
                                            Entity entOrderUpdate = new Entity(ent.LogicalName, ent.Id);
                                            entOrderUpdate["hil_sapsyncmessage"] = "Discount amount is negative";
                                            entOrderUpdate["hil_issynctosap"] = false;
                                            _service.Update(entOrderUpdate);
        ...
        public WarrantyDate GetWarrantyStartAndEndDate(Guid AssetId, Guid CustomerId, Guid OrderId)
        {
            ...
            string fetch = $@"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
                          <entity name='hil_unitwarranty'>
                            ...
                            <order attribute='createdon' descending='true' />
                            <filter type='and'>
                              <condition attribute='hil_order' operator='eq' value='{OrderId}' />
                              <condition attribute='hil_customer' operator='eq' value='{CustomerId}' />
                              <condition attribute='hil_customerasset' operator='eq' value='{AssetId}' />
                              <condition attribute='statecode' operator='eq' value='0' />
                            </filter>
                          </entity>
                        </fetch>";
            EntityCollection entColl = _service.RetrieveMultiple(new FetchExpression(fetch));
            if (entColl.Entities.Count > 0)
            {
                warrantyDate.StartDate = entColl.Entities[0].GetAttributeValue<DateTime>("hil_warrantystartdate").AddMinutes(330).ToString("yyyy-MM-dd");
                warrantyDate.EndDate = entColl.Entities[0].GetAttributeValue<DateTime>("hil_warrantyenddate").AddMinutes(330).ToString("yyyy-MM-dd");
            }
            return warrantyDate;
        }
        ...
        private ActionResponse CreateMediaGallary(Entity SalesOrder, string BILLING_NUMBER)
        {
            ...
            try
            {
                ...
                mediagallery["hil_name"] = $"ALaCarte_Invoice_{SalesOrder.Id.ToString()}";
