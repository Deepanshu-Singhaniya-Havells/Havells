...
namespace OMSMail
{
    public class CaseResolution
    {
        ...
        public CaseResolution(IOrganizationService _service)
        {
            service = _service;
        }

        public void ExecuteCaseResolution()
        {
            try
            {
                string logicalNameOfBPF = "phonetocaseprocess";
                //Entity entity = new Entity("incident");
                Entity entity = service.Retrieve("incident", new Guid("aab09422-de69-4596-b0a7-03758f0834dd"), new Microsoft.Xrm.Sdk.Query.ColumnSet("casetypecode"));
                if (entity.Contains("casetypecode") && entity["casetypecode"] != null)
                {
                    int casetyepe = entity.GetAttributeValue<OptionSetValue>("casetypecode").Value;
                    if (casetyepe == 1)
                    {
                ...
                {
                    Console.WriteLine("Case type not found...");
                }
            }
            catch (Exception ex)
            {
