...
namespace TestApp
{
    ...
    public class MRNEntry
    {
        ...
        public MRNHeader AddtoViewList(MRNEntry _productInfo, IOrganizationService service)
        {
            ...
            try
            {
                if (service != null)
                {
                    ...
                    qrExp.Criteria.AddCondition(new ConditionExpression("ownerid", ConditionOperator.Equal, _productInfo.Technician));
                    qrExp.Criteria.AddCondition(new ConditionExpression("hil_customercode", ConditionOperator.Equal, _productInfo.CustomerGuId));
                    qrExp.Criteria.AddCondition(new ConditionExpression("hil_mrnstatus", ConditionOperator.Equal, 1));
                    qrExp.Criteria.AddCondition(new ConditionExpression("createdon", ConditionOperator.Today));
                    entCol = service.RetrieveMultiple(qrExp);
                    if (entCol.Entities.Count == 0)
                    {
                        ...
                        ent["hil_mrnstatus"] = new OptionSetValue(1);
