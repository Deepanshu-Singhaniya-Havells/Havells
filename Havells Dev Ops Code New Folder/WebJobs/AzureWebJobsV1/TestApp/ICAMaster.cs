...
namespace TestApp
{
    public class ICAMaster
    {
        public void CreateICAMasters(IOrganizationService _service)
        {
            #region Excel Sheet Columns
            string _callSubType = "";
            string _prodSubCatg = "";
            string _natureOfComlaint = "";
            string _observation = "";
            string _cause = "";
            string _causeAction = ""; //
            Decimal _amount = 0;
            ...
            #region Validating Product SubCategory
            ...
            #region Validating Call SubType
            ...
            #endregion
            if (!_errorOccured) {
                _natureOfComlaintGuId = GetNatureOfComplaint(_callSubTypGuId, _prodSubCatgGuId, _natureOfComlaint, _service, out _retMsg);
                if (_natureOfComlaintGuId != Guid.Empty)
                {
                    if (_retMsg != "OK") { _errorRemarks.AppendLine(_retMsg); }
                    _observationGuId = GetObservation(_prodSubCatgGuId, _observation, _service, out _retMsg);
                    if (_observationGuId != Guid.Empty) {
                        if (_retMsg != "OK") { _errorRemarks.AppendLine(_retMsg); }
                        _causeGuId = GetCause(_observationGuId, _prodSubCatgGuId, _cause, _service, out _retMsg);
                        if (_causeGuId != Guid.Empty)
                        {
                            if (_retMsg != "OK") { _errorRemarks.AppendLine(_retMsg); }
                            _causeActionGuId = GetService(_causeAction, _amount, _service, out _retMsg);
                            if (_causeActionGuId != Guid.Empty)
                            {
                                if (_retMsg != "OK") { _errorRemarks.AppendLine(_retMsg); }
                                Guid _causeServiceGuId = GetCauseService(_causeGuId, _causeActionGuId, "", _service, out _retMsg);
            ...
            if (_errorOccured) {
                Console.WriteLine(_errorRemarks.ToString());
            }
        }
        ...
        public Guid GetCauseService(Guid _causeGuId, Guid _serviceGuId, string _causeServiceName, IOrganizationService _service, out string _retMsg)
        {
            ...
            try
            {
                ...
                else
                {
                   ...
                   // entObj.Attributes["msdyn_lineorder"] = 1;
