...
namespace Havells.CRM.AMCInvoiceSync
{
    public class GetPendingPaymentStatus
    {
        public static void getPaymentStatus(IOrganizationService service)
        {
            ...
            Query.Criteria.AddCondition("createdon", ConditionOperator.OnOrAfter, new DateTime(2022, 11, 30));
            ...
            foreach (Entity entity in paymentCollection.Entities)
            {
                ...
                updatePaymentStatusforJob(transactionID, service, intConfig.uri, authInfo);
        ...
        {
            IntegrationConfig intConfig = Program.IntegrationConfiguration(service, "Send Payment Link");
            intConfig.uri = "https://middlewareqa.havells.com:50001/RESTAdapter/PayUBiz/TransactionAPI";
            ...
            Query.Criteria.AddCondition("msdyn_name", ConditionOperator.Like, "%INV-2023-001143%");
            Query.Orders.Add(new OrderExpression("createdon", OrderType.Descending));
            EntityCollection paymentCollection = service.RetrieveMultiple(Query);
        ...
        static void updatePaymentStatusforJob(String transactionID, IOrganizationService service, string URL, string authInfo)
        {
            try
            {
                StatusRequest req = new StatusRequest();
                ...
                if (Found.Entities.Count > 0)
                {
                    ...
                    if (Found[0].Contains("hil_job"))
                    {
                        ...
                        else if (obj.transaction_details[0].status == "pending")
                        {
                            ...
                            paymentstatus = (4);
                        updatePaymentinJob(service, Found[0].GetAttributeValue<EntityReference>("hil_job").Id, paymentstatus);
                    }
                }
            }
            catch (Exception ex)
        ...
        static void updatePaymentinJob(IOrganizationService service, Guid JobId, int paymentstatus)
        {
