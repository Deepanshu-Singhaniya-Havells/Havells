...
namespace Havells.CRM.PerformaInvoice
{
    ...
    public class ClaimPerformaNew
    {
        ...
        public IOrganizationService ConnectToCRM(string connectionString)
        {
        ...
        public void GenerateAnnexureUrl(IOrganizationService _service, EntityCollection ClaimInvoiceCollection)
        {
            string AnnextureUrl = string.Empty;
            int total = ClaimInvoiceCollection.Entities.Count;
            int done = 0;
            if (_service != null)
            {
                try
                {
                    Console.WriteLine("Total Performa Invoice Count is " + ClaimInvoiceCollection.Entities.Count);
                    foreach (Entity entity in ClaimInvoiceCollection.Entities)
                    {
                        done++;
                        try
                        {
                            Console.WriteLine("************************************************************************");
                            Console.WriteLine("Invoice Genrating for PI is " + entity["hil_name"]);

                            //if (entity["hil_name"].ToString() != "CL-00035468")
                            //    continue;

                            EntityReference _performaInvoiceRef = entity.ToEntityReference();
                            ...
                            {
                                Console.WriteLine("Invoice Genrating for PI is " + entity["hil_name"]);
                                EntityCollection ClaimLinesInvoiceCollection = new EntityCollection();
                                EntityCollection entityCollection = new EntityCollection();
                                int pageNo = 1;

                            hil_claimline:
                                string ClaimfetchXml = $@"<fetch version='1.0' page='" + pageNo + $@"' output-format='xml-platform' mapping='logical' distinct='false'>
                          <entity name='hil_claimline'>
                            ...
                            <filter type='and'>
                                <condition attribute='hil_claimheader' operator='eq' value='{_performaInvoiceRef.Id}' />
                            ...
                            <link-entity name='hil_claimcategory' from='hil_claimcategoryid' to='hil_claimcategory' visible='false' link-type='outer' alias='ac'>
                              <attribute name='hil_claimtype' />
                                ...
                                entityCollection = _service.RetrieveMultiple(new FetchExpression(ClaimfetchXml));
                                ClaimLinesInvoiceCollection.Entities.AddRange(entityCollection.Entities.ToArray());
                                ...
                                Console.WriteLine("Claim Line Count is " + ClaimLinesInvoiceCollection.Entities.Count);
                                ...
                                foreach (Entity row in ClaimLinesInvoiceCollection.Entities)
                                {
                                    ...
                                    dr["JobId"] = row.Contains("hil_jobid") ? row.GetAttributeValue<EntityReference>("hil_jobid").Name.ToString() : "";
                                    dt.Rows.Add(dr);
                                }

                                pageNo = 1;
                                EntityCollection ClaimCategoryCollection = new EntityCollection();
                                entityCollection = new EntityCollection();
                                hil_claimcategory:
                                string fetchCategoryXml = @"<fetch version='1.0' page='" + pageNo + @"' output-format='xml-platform' mapping='logical' distinct='false'>
                          <entity name='hil_claimcategory'>
                            ...
                            <link-entity name='hil_claimtype' from='hil_claimtypeid' to='hil_claimtype' visible='false' link-type='outer' alias='ab'>
                              <attribute name='hil_displayindex' />
                                ...
                                entityCollection = _service.RetrieveMultiple(new FetchExpression(fetchCategoryXml));
                                ClaimCategoryCollection.Entities.AddRange(entityCollection.Entities.ToArray());

                                if (entityCollection.Entities.Count == 5000)
                                {
                                    pageNo++;
                                    goto hil_claimcategory;
                                }

                                Console.WriteLine("Claim Line Count is " + ClaimCategoryCollection.Entities.Count);
                                ...
                                AnnextureUrl = documentHtml(_service, entity, dt, Categorydt, _performaInvoiceRef);
                            }
                ...
                {
                    Console.WriteLine("D365 Service Unavailable. " + ex.Message);
        ...
        static string documentHtml(IOrganizationService _service, Entity ClaimInvoiceCollection, DataTable Ordt, DataTable Catdt, EntityReference performaInvoice)
        {
            Console.WriteLine("Document Genration is in progress.");
            string AnnextureURL = "";
            ...
            try
            {
                if (_service != null)
                {
                    ...
                    {
                        if (ClaimInvoiceCollection.Contains("hil_name"))
                        {
                            hil_name = ClaimInvoiceCollection.GetAttributeValue<string>("hil_name");
                        }
                        if (ClaimInvoiceCollection.Contains("hil_franchisee"))
                        {
                            hil_franchisee = ClaimInvoiceCollection.GetAttributeValue<EntityReference>("hil_franchisee").Name;
                        }
                        if (ClaimInvoiceCollection.Contains("ab.accountnumber"))
                        {
                            accountnumber = ClaimInvoiceCollection.GetAttributeValue<AliasedValue>("ab.accountnumber").Value.ToString();
                        }
                        if (ClaimInvoiceCollection.Contains("ab.hil_vendorcode"))
                        {
                            hil_vendorcode = ClaimInvoiceCollection.GetAttributeValue<AliasedValue>("ab.hil_vendorcode").Value.ToString();
                        }
                        if (ClaimInvoiceCollection.Contains("ab.hil_category"))
                        {
                            hil_category = ClaimInvoiceCollection.FormattedValues["ab.hil_category"].ToString();
                        }
                        if (ClaimInvoiceCollection.Contains("ab.hil_district"))
                        {
                            hil_district = ((EntityReference)ClaimInvoiceCollection.GetAttributeValue<AliasedValue>("ab.hil_district").Value).Name.ToString();
                        }
                        if (ClaimInvoiceCollection.Contains("ab.hil_area"))
                        {
                            hil_area = ((EntityReference)ClaimInvoiceCollection.GetAttributeValue<AliasedValue>("ab.hil_area").Value).Name.ToString();
                        }
                        if (ClaimInvoiceCollection.Contains("ab.hil_salesoffice"))
                        {
                            hil_salesoffice = ((EntityReference)ClaimInvoiceCollection.GetAttributeValue<AliasedValue>("ab.hil_salesoffice").Value).Name.ToString();
                        }
                        if (ClaimInvoiceCollection.Contains("hil_fiscalmonth"))
                        {
                            hil_fiscalmonth = ClaimInvoiceCollection.GetAttributeValue<EntityReference>("hil_fiscalmonth").Name;
                        }
                        if (ClaimInvoiceCollection.Contains("hil_syncdoneon"))
                        {
                            hil_syncdoneon = ClaimInvoiceCollection.GetAttributeValue<DateTime>("hil_syncdoneon").AddMinutes(330);
                        }
                        if (ClaimInvoiceCollection.Contains("ac.hil_fromdate"))
                        {
                            hil_fromdate = Convert.ToDateTime(ClaimInvoiceCollection.GetAttributeValue<AliasedValue>("ac.hil_fromdate").Value).AddMinutes(330);
                        }
                        if (ClaimInvoiceCollection.Contains("ac.hil_todate"))
                        {
                            hil_todate = Convert.ToDateTime(ClaimInvoiceCollection.GetAttributeValue<AliasedValue>("ac.hil_todate").Value).AddMinutes(330);
                        }
                        if (ClaimInvoiceCollection.Contains("hil_sapponumber"))
                        {
                            hil_sapponumber = ClaimInvoiceCollection.GetAttributeValue<string>("hil_sapponumber");
                        }
                        if (ClaimInvoiceCollection.Contains("modifiedon"))
                        {
                            modifiedon = ClaimInvoiceCollection.GetAttributeValue<DateTime>("modifiedon").AddMinutes(330);
                        }
                        if (ClaimInvoiceCollection.Contains("hil_approvedon"))
                        {
                            hil_approvedon = ClaimInvoiceCollection.GetAttributeValue<DateTime>("hil_approvedon").AddMinutes(330);
                        }

                        DateTime? approvedon = hil_approvedon == null ? modifiedon : hil_approvedon;
                        ...
                        <div class='box'>
                           ...
                           <table class='firstTable' >  
                            ...
                            <tr>
                                <td>Approval Date</td>
                                <td>" + (approvedon == null ? "" : String.Format("{0:dd/MM/yyyy}", approvedon)) + @"</td>
                            </tr>
	                         <tr>
                        ...
                        DataTable dt = Ordt.AsEnumerable()
                              .GroupBy(r => new
                              {
                        ...
                        string[] TotalClaimableCol = new string[] { "ActivityCode", "ProductCategory", "CallSubType", "JobId" };
                        DataView dvTotalClaimable = new DataView(Ordt);
                        dvTotalClaimable.Sort = "ActivityCode ASC";
                        DataTable dtTotalClaimable = dvTotalClaimable.ToTable(true, TotalClaimableCol);

                        for (int j = 0; j < distinctValues.Rows.Count; j++)
                        {
                            string tdvalues = setvalue(dtTotalClaimable, dt, distinctValues.Rows[j], Catdt);
                            body = body + @"<tr>
                        ...
                        string strbottom = setBottomvalue(dtTotalClaimable, dt, Catdt);
                        body = body + strbottom;
                        ...
                        string _fileName = "ClaimInvoice_" + ClaimInvoiceCollection.Id + ".pdf";
                        Console.WriteLine("Invoice against Claim : " + ClaimInvoiceCollection.GetAttributeValue<string>("msdyn_name"));

                        Console.WriteLine("Invoice template is created.");
                        if (html != null)
                        {
        ...
        public static string setvalue(DataTable Ordt, DataTable dt, DataRow dr, DataTable Catdt)
        {
            bool IsGenTd;
            string body = "";
            ...
            int k = 0;
            for (int i = 0; i < Ordt.Rows.Count; i++)
            {
                if (dr["ActivityCode"].ToString() == Ordt.Rows[i]["ActivityCode"].ToString() && dr["ProductCategory"].ToString() == Ordt.Rows[i]["ProductCategory"].ToString() && dr["CallSubType"].ToString() == Ordt.Rows[i]["CallSubType"].ToString())
                {
        ...
        public static string setBottomvalue(DataTable ordt, DataTable dt, DataTable Catdt)
        {
            bool IsBottomGenTd;
            int OverAllTotal = 0;
            string BottomTotal = "<tr><td colspan='3' style='border: none;'></td><td style='font-size:13px; font-weight:bold;'>" + ordt.Rows.Count.ToString() + "</td>";
        ...
        public static string HtmltoPdf(string _fileName, string htmlBody)
        {
            ...
            try
            {
                ...
                Console.WriteLine("Document is Created and Uploaded in Blob with URL " + blobUrl);
            }
            catch (Exception ex)
            {
        ...
        static string Upload(string fileName, byte[] fileContent, string containerName)
        {
            ...
            {
                throw new Exception("Upload: " + ex.Message);
            }
        ...
        public static void createAttachment(IOrganizationService service, EntityReference regardingEntity, string URL)
        {
            ...
            Entity PI = new Entity(regardingEntity.LogicalName);
            PI.Id = regardingEntity.Id;
            PI["hil_isperformainvoicecreated"] = true;
            service.Update(PI);

            Console.WriteLine("Flag Updated");
        }
