...
namespace Approvalcreate
{
    ...
    public class CreateApproval
    {
        ...
        public static void CreateOCLPriceApproval(IOrganizationService service, ITracingService tracingService, string entityName, string entityId, int purpose, int entityObjectType)
        {
            ...
            var approvalLines1 = approvalLines.OrderBy(x => x.ApprovalName).ThenBy(x => x.Level).ToList();
            Console.WriteLine("******************************************************");
            foreach (ApprovalsClass x in approvalLines)
            {
                Console.Write("ApproverName " + x.ApprovalName);
                Console.Write(" | ApproverID " + x.ApproverID);
                Console.Write(" | Level " + x.Level);
                Console.Write(" | Material Group ID " + x.MAterialGroupName);
                Console.Write(" | Material Group Name " + x.MAterialGroupName);
                Console.WriteLine();
            }
            Console.WriteLine("***************");
            foreach (ApprovalsClass guid in approvalLines1)
            {
                Console.WriteLine(guid.ApprovalName);
            }
            List<Guid> approverList = approvalLines1.Select(x => x.ApproverID).Distinct().ToList();
            ...
            createApprovalsforOCLPrice(service, approvalLines123, targetEntity);

            submitForApproval(service, targetEntity.ToEntityReference(), approverList);
        ...
        public static void createApprovalsforOCLPrice(IOrganizationService service, List<ApprovalsClass>[] approvalHeadrs, Entity target)
        {
            ...
            foreach (List<ApprovalsClass> approvalsClass in approvalHeadrs)
            {
                ...
                Console.WriteLine("Approval Header Created..");
                createApprovalsLinesforOCLPrice(service, approvalsClass, target, approvalHeader, ("Approvals for MG : " + MGList.Substring(0, MGList.Length - 2)), approver);
            }
        }
        public static void createApprovalsLinesforOCLPrice(IOrganizationService service, List<ApprovalsClass> approvalHeadrs, Entity target, EntityReference approvalHeader, string headerName, EntityReference approver)
        {
            foreach (ApprovalsClass approvalsClass in approvalHeadrs)
            {
                ...
                foreach (Entity entity in oclPrd.Entities)
                {
                    ...
                    Console.WriteLine("Approval Line Created..");
                }
            }
        }
        public static void submitForApproval(IOrganizationService service, EntityReference target, List<Guid> approvalList)
        {
            Console.WriteLine("**********************");
            ...
            foreach (Guid approverId in approvalList)
            {
                ...
                if (sendForApproval)
                {
                    ...
                    Console.WriteLine("header Status changed");
                }
            }
        }
        ...
        public static void submitForApprovalAfterActionTaken(IOrganizationService service, EntityReference target, List<Guid> approvalList, ITracingService tracingService)
        {
            ...
            foreach (Guid approverId in approvalList)
            {
                ...
                Console.WriteLine("check for Pendincinmg");
                if (sendForApproval)
                {
                    EntityReference approvalheader = entityCollection[0].GetAttributeValue<EntityReference>("hil_approval");
                    Entity aplHead = new Entity(approvalheader.LogicalName, approvalheader.Id);
