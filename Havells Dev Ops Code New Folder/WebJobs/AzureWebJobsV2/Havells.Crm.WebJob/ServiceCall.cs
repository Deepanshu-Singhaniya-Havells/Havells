...
namespace Havells.Crm.WebJob
{
    public static class servicecall
    {
        public static void Execute(IOrganizationService service)
        {
            ...
            EntityReference hil_address = new EntityReference();
            EntityReference hil_consumer = new EntityReference();
            try
            {

                {
                    Entity entity = service.Retrieve("hil_bulkjobsuploader", new Guid("f082466c-b409-ed11-82e6-6045bdac5e8e"), new ColumnSet(true));
                    DateTime? hil_expecteddeliverydate = null;
                    if (entity.GetAttributeValue<int>("hil_sourceofjob") == 18)
                    {

                        #region Getting Data from Excel Sheet Columns
                        hil_address = entity.GetAttributeValue<EntityReference>("hil_address");

                        hil_consumer = entity.GetAttributeValue<EntityReference>("hil_consumer");
                        ...
                        if (inputsValidated)
                        {
                            ...
                            entConsumer = service.Retrieve("contact", hil_consumer.Id, new ColumnSet("fullname", "emailaddress1", "contactid"));

                            _customerGuid = entConsumer.Id;
                            ...
                            if (!_duplicateWO)
                            {
                                if (_customerGuid != Guid.Empty)
                                {
                                    ...
                                    erAddress = hil_address;

                                }

                                if (erAddress == null)
                                {
                                    entity["hil_description"] = "Address is required.";
                                ...
                                if (entConsumer != null && erAddress != null)
                                {
                                    try
                                    {
                                        ...
                                        if (entTemp != null)
                                        {
                                            ...
                                            enPMSWorkorder.hil_SourceofJob = new OptionSetValue(18); // {SourceofJob:"Excel Upload"}
                                    ...
                                    {
                                        entity["hil_description"] = "Error in Job Creation " + ex.Message;
                                        entity["hil_jobstatus"] = false;
                                    }
                                }
                                else
        ...
        public static EntityReference GetCustomerAddress(IOrganizationService service, Guid customerGuid)
        {
